...
export function buildApiServiceCatalogItems({
  ...
  return [
    ...
    {
      method: "GET",
      endpoint: "/api/v1/trading/routes",
      product: "執行路由查詢",
      status: tradeTickets.length ? portfolioStatus : "watch",
      owner: cleanDecisionOwner,
      consumer: "交易 / 營運 / 稽核",
      input: "workspace_id、portfolio_id、batch_id",
      output: "route_id、venue、route_state、成本、approval_decision",
      serviceLevel: "每次送簽與路由模擬後可查最新狀態",
      action: tradeTickets.length ? "可查詢 route audit trail" : "先建立交易票與 route 模擬",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/routes",
      ...
      owner: cleanDecisionOwner,
      consumer: "交易 / 營運 / 稽核",
      input: "ExecutionRouteSyncPayload、workspace_id、portfolio_id、batch_id",
      output: "寫入筆數、資料表、錯誤明細",
      serviceLevel: "模擬 route 必須可回查，不直接送出真實委託",
      action: tradeTickets.length ? "可把模擬 route 寫入 BigQuery" : "先建立交易票與 route 模擬",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/route-events",
      product: "路由事件查詢",
      status: tradeTickets.length ? portfolioStatus : "watch",
      owner: cleanDecisionOwner,
      consumer: "交易 / 營運 / 稽核",
      input: "workspace_id、portfolio_id、batch_id、route_id",
      output: "event_id、event_type、event_status、broker_mode、evidence",
      serviceLevel: "每次 route 模擬後可查 broker boundary 事件",
      action: tradeTickets.length ? "可查詢 route event history" : "先建立 route 模擬與事件",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/route-events",
      ...
      owner: cleanDecisionOwner,
      consumer: "交易 / 營運 / 稽核",
      input: "ExecutionRouteEventSyncPayload、workspace_id、portfolio_id、batch_id",
      output: "寫入筆數、資料表、錯誤明細",
      serviceLevel: "broker boundary 與 paper ack 必須可稽核",
      action: tradeTickets.length ? "可把 route event history 寫入 BigQuery" : "先建立 route 模擬與事件",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/fills",
      product: "成交回報查詢",
      status: tradeTickets.length ? portfolioStatus : "watch",
      owner: cleanDecisionOwner,
      consumer: "交易 / 營運 / 復盤 / 稽核",
      input: "workspace_id、portfolio_id、batch_id",
      output: "fill_id、成交金額、未成交、滑價、手續費、成本",
      serviceLevel: "成交回填後可查最新狀態，不依賴前端暫存",
      action: tradeTickets.length ? "可查詢 execution fill ledger" : "先建立交易票與成交回報",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/fills",
      ...
      owner: cleanDecisionOwner,
      consumer: "交易 / 營運 / 復盤 / 稽核",
      input: "ExecutionFillSyncPayload、workspace_id、portfolio_id、batch_id",
      output: "寫入筆數、資料表、錯誤明細",
      serviceLevel: "模擬或人工成交回報必須可落庫與稽核",
      action: tradeTickets.length ? "可把成交回報寫入 BigQuery" : "先建立交易票與成交回報",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/post-trade-attribution",
      product: "交易後歸因查詢",
      status: tradeTickets.length ? portfolioStatus : "watch",
      owner: cleanDecisionOwner,
      consumer: "營運 / 風控 / 投委會 / 稽核",
      input: "workspace_id、portfolio_id、batch_id",
      output: "復盤指標、狀態、門檻、說明、殘單曝險",
      serviceLevel: "成交回填後需可查本輪交易復盤指標",
      action: tradeTickets.length ? "可查詢 post-trade attribution ledger" : "先建立成交回報與復盤指標",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/post-trade-attribution",
      ...
      owner: cleanDecisionOwner,
      consumer: "營運 / 風控 / 投委會 / 稽核",
      input: "PostTradeAttributionSyncPayload、workspace_id、portfolio_id、batch_id",
      output: "寫入筆數、資料表、錯誤明細",
      serviceLevel: "每輪成交復盤需可落庫，不能只存在前端畫面",
      action: tradeTickets.length ? "可把 post-trade attribution 寫入 BigQuery" : "先建立成交回報與復盤指標",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/platform-exceptions",
      product: "例外事項查詢",
      status: tradeTickets.length ? portfolioStatus : "watch",
      owner: cleanRiskOwner,
      consumer: "營運 / 風控 / 客戶成功 / 稽核",
      input: "workspace_id、portfolio_id、batch_id",
      output: "例外來源、負責人、優先級、期限、狀態、下一步",
      serviceLevel: "每輪交易需可查目前未關閉例外",
      action: tradeTickets.length ? "可查詢 platform exception queue" : "先建立例外事項總控台",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/platform-exceptions",
      ...
      owner: cleanRiskOwner,
      consumer: "營運 / 風控 / 客戶成功 / 稽核",
      input: "PlatformExceptionSyncPayload、workspace_id、portfolio_id、batch_id",
      output: "寫入筆數、資料表、錯誤明細",
      serviceLevel: "阻擋與觀察項需能跨裝置回查並交接",
      action: tradeTickets.length ? "可把例外 queue 寫入 BigQuery" : "先建立例外事項總控台",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/sla-escalations",
      product: "SLA 升級查詢",
      status: tradeTickets.length ? portfolioStatus : "watch",
      owner: cleanRiskOwner,
      consumer: "營運 / 風控 / 客戶成功 / 稽核",
      input: "workspace_id、portfolio_id、batch_id",
      output: "升級層級、負責人、觸發項、時限、升級路徑、動作",
      serviceLevel: "例外產生後需可查 SLA 處理時限",
      action: tradeTickets.length ? "可查詢 SLA escalation matrix" : "先建立例外事項與 SLA 規則",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/sla-escalations",
      ...
      owner: cleanRiskOwner,
      consumer: "營運 / 風控 / 客戶成功 / 稽核",
      input: "SlaEscalationSyncPayload、workspace_id、portfolio_id、batch_id",
      output: "寫入筆數、資料表、錯誤明細",
      serviceLevel: "L1/L2/L3 升級紀錄需可交接與稽核",
      action: tradeTickets.length ? "可把 SLA escalation 寫入 BigQuery" : "先建立例外事項與 SLA 規則",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/operating-kri",
      product: "營運 KRI 查詢",
      status: tradeTickets.length ? portfolioStatus : "watch",
      owner: cleanRiskOwner,
      consumer: "營運 / 風控 / 投委會 / 稽核",
      input: "workspace_id、portfolio_id、batch_id",
      output: "KRI 指標、狀態、目前值、門檻、負責人、說明",
      serviceLevel: "每輪投資與交易決策需可查營運風險狀態",
      action: tradeTickets.length ? "可查詢 operating KRI board" : "先建立營運 KRI 指標",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/operating-kri",
      ...
      owner: cleanRiskOwner,
      consumer: "營運 / 風控 / 投委會 / 稽核",
      input: "OperatingKriSyncPayload、workspace_id、portfolio_id、batch_id",
      output: "寫入筆數、資料表、錯誤明細",
      serviceLevel: "資料品質、例外、SLA 與復盤狀態需可落庫與追溯",
      action: tradeTickets.length ? "可把 operating KRI 寫入 BigQuery" : "先建立營運 KRI 指標",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/decision-funnel",
      product: "決策漏斗查詢",
      status: tradeTickets.length ? portfolioStatus : "watch",
      owner: cleanRiskOwner,
      consumer: "研究 / 投委會 / 營運 / 稽核",
      input: "workspace_id、portfolio_id、batch_id",
      output: "階段、狀態、數值、轉換率、負責人、說明",
      serviceLevel: "資料到交易的每個決策階段需可回查",
      action: tradeTickets.length ? "可查詢 decision funnel board" : "先建立決策漏斗階段",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/decision-funnel",
      ...
      owner: cleanRiskOwner,
      consumer: "研究 / 投委會 / 營運 / 稽核",
      input: "DecisionFunnelSyncPayload、workspace_id、portfolio_id、batch_id",
      output: "寫入筆數、資料表、錯誤明細",
      serviceLevel: "每輪研究、配置、交易、成交與放行狀態需可落庫",
      action: tradeTickets.length ? "可把 decision funnel 寫入 BigQuery" : "先建立決策漏斗階段",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/market-alerts",
      product: "市場警示查詢",
      status: tradeTickets.length ? portfolioStatus : "watch",
      owner: cleanRiskOwner,
      consumer: "營運 / 風控 / 投委會 / 客戶成功",
      input: "workspace_id、portfolio_id、batch_id",
      output: "警示來源、標題、狀態、優先級、負責人、依據、動作",
      serviceLevel: "每輪資料、交易與營運警示需可回查",
      action: tradeTickets.length ? "可查詢 market alert event stream" : "先建立市場警示事件",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/market-alerts",
      ...
      owner: cleanRiskOwner,
      consumer: "營運 / 風控 / 投委會 / 客戶成功",
      input: "MarketAlertSyncPayload、workspace_id、portfolio_id、batch_id",
      output: "寫入筆數、資料表、錯誤明細",
      serviceLevel: "警示中心需可落庫，支援後續分派、runbook 與稽核",
      action: tradeTickets.length ? "可把 market alert events 寫入 BigQuery" : "先建立市場警示事件",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/market-alert-owner-queues",
      product: "警示分派查詢",
      status: tradeTickets.length ? portfolioStatus : "watch",
      owner: cleanRiskOwner,
      consumer: "營運 / 風控 / 客戶成功 / 值班主管",
      input: "workspace_id、portfolio_id、batch_id",
      output: "負責人、優先級、事件統計、主要來源、下一步",
      serviceLevel: "警示分派需可查目前 owner 負載與首要處理項",
      action: tradeTickets.length ? "可查詢 market alert owner queue" : "先建立市場警示分派",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/market-alert-owner-queues",
      ...
      owner: cleanRiskOwner,
      consumer: "營運 / 風控 / 客戶成功 / 值班主管",
      input: "MarketAlertOwnerQueueSyncPayload、workspace_id、portfolio_id、batch_id",
      output: "寫入筆數、資料表、錯誤明細",
      serviceLevel: "owner queue 需可落庫，支援交班與責任歸屬",
      action: tradeTickets.length ? "可把 market alert owner queue 寫入 BigQuery" : "先建立市場警示分派",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/market-alert-runbooks",
      product: "警示 Runbook 查詢",
      status: tradeTickets.length ? portfolioStatus : "watch",
      owner: cleanRiskOwner,
      consumer: "營運 / 風控 / 客戶成功 / 值班主管",
      input: "workspace_id、portfolio_id、batch_id",
      output: "來源、標題、期限、觸發、檢查、修復、驗收、升級",
      serviceLevel: "每個高優先或觀察事件需有可追溯處理手冊",
      action: tradeTickets.length ? "可查詢 market alert runbook" : "先建立市場警示 runbook",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/market-alert-runbooks",
      product: "警示 Runbook 同步",
      status: tradeTickets.length ? portfolioStatus : "watch",
      owner: cleanRiskOwner,
      consumer: "營運 / 風控 / 客戶成功 / 值班主管",
      input: "MarketAlertRunbookSyncPayload、workspace_id、portfolio_id、batch_id",
      output: "寫入筆數、資料表、錯誤明細",
...
export function buildApiContractBlueprintItems({
  ...
  return [
    ...
    },
    {
      ...
      action: "approval gate 與 route status 已接；下一步補 broker API 邊界",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/routes",
      ...
      auth: "internal",
      requestSchema: "{ portfolio_id?: string; batch_id?: string }",
      responseSchema: "ExecutionRouteRow[]",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/routes"),
      owner: cleanDecisionOwner,
      breakingRisk: "高：route_id、venue、route_state 與成本欄位需可追溯",
      action: "可查 BigQuery route audit trail；下一步補 route 事件歷史",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/routes",
      ...
      version: "v1",
      auth: "internal",
      requestSchema: "ExecutionRouteSyncPayload & { workspace_id: string; portfolio_id: string; batch_id: string }",
      responseSchema: "ExecutionRouteSyncResult",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/routes"),
      owner: cleanDecisionOwner,
      breakingRisk: "高：route_state、venue、approval_decision 與 idempotency_key 需可追溯",
      action: "僅同步模擬 route，不連真實下單；broker boundary 已進入事件歷史",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/route-events",
      ...
      version: "v1",
      auth: "internal",
      requestSchema: "{ portfolio_id?: string; batch_id?: string; route_id?: string }",
      responseSchema: "ExecutionRouteEventRow[]",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/route-events"),
      owner: cleanDecisionOwner,
      breakingRisk: "高：event_id、event_type、event_status 與 broker_mode 需可追溯",
      action: "可查 BigQuery route event history；下一步補 broker adapter 合約測試",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/route-events",
      ...
      version: "v1",
      auth: "internal",
      requestSchema: "ExecutionRouteEventSyncPayload & { workspace_id: string; portfolio_id: string; batch_id: string }",
      responseSchema: "ExecutionRouteEventSyncResult",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/route-events"),
      owner: cleanDecisionOwner,
      breakingRisk: "高：broker boundary、paper ack 與 idempotency_key 需可追溯",
      action: "僅同步事件歷史，不連真實券商；下一步補 broker adapter 合約測試",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/fills",
      ...
      auth: "internal",
      requestSchema: "{ portfolio_id?: string; batch_id?: string }",
      responseSchema: "ExecutionFillRow[]",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/fills"),
      owner: cleanDecisionOwner,
      breakingRisk: "高：fill_id、ticket_id、成本與 cash impact 欄位需可追溯",
      action: "可查 BigQuery execution fill ledger；下一步補 post-trade attribution 落庫",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/fills",
      ...
      version: "v1",
      auth: "internal",
      requestSchema: "ExecutionFillSyncPayload & { workspace_id: string; portfolio_id: string; batch_id: string }",
      responseSchema: "ExecutionFillSyncResult",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/fills"),
      owner: cleanDecisionOwner,
      breakingRisk: "高：filled_notional、unfilled_notional、slippage 與 commission 需可稽核",
      action: "僅同步模擬或人工回報，不連真實券商；下一步補成本異常告警",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/post-trade-attribution",
      ...
      auth: "internal",
      requestSchema: "{ portfolio_id?: string; batch_id?: string }",
      responseSchema: "PostTradeAttributionItem[]",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/post-trade-attribution"),
      owner: cleanDecisionOwner,
      breakingRisk: "高：metric_key、status、threshold 與 residual impact 需可追溯",
      action: "可查 BigQuery post-trade attribution ledger；下一步補例外 queue 落庫",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/post-trade-attribution",
      ...
      version: "v1",
      auth: "internal",
      requestSchema: "PostTradeAttributionSyncPayload & { workspace_id: string; portfolio_id: string; batch_id: string }",
      responseSchema: "PostTradeAttributionSyncResult",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/post-trade-attribution"),
      owner: cleanDecisionOwner,
      breakingRisk: "高：歸因指標代表復盤結論，欄位與狀態需向後相容",
      action: "把成交後復盤固定成可稽核資料；下一步補例外 queue 落庫",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/platform-exceptions",
      ...
      auth: "internal",
      requestSchema: "{ portfolio_id?: string; batch_id?: string }",
      responseSchema: "PlatformExceptionItem[]",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/platform-exceptions"),
      owner: cleanRiskOwner,
      breakingRisk: "高：priority、status、owner 與 next_action 會影響營運交接",
      action: "可查 BigQuery platform exception queue；下一步補 SLA escalation 落庫",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/platform-exceptions",
      ...
      version: "v1",
      auth: "internal",
      requestSchema: "PlatformExceptionSyncPayload & { workspace_id: string; portfolio_id: string; batch_id: string }",
      responseSchema: "PlatformExceptionSyncResult",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/platform-exceptions"),
      owner: cleanRiskOwner,
      breakingRisk: "高：例外 queue 是營運閉環依據，欄位需向後相容",
      action: "把阻擋與觀察項固定成可交接資料；下一步補 SLA escalation 落庫",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/sla-escalations",
      ...
      auth: "internal",
      requestSchema: "{ portfolio_id?: string; batch_id?: string }",
      responseSchema: "SlaEscalationItem[]",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/sla-escalations"),
      owner: cleanRiskOwner,
      breakingRisk: "高：tier、due、escalation_path 會影響營運升級規則",
      action: "可查 BigQuery SLA escalation matrix；下一步補營運 KRI 落庫",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/sla-escalations",
      ...
      version: "v1",
      auth: "internal",
      requestSchema: "SlaEscalationSyncPayload & { workspace_id: string; portfolio_id: string; batch_id: string }",
      responseSchema: "SlaEscalationSyncResult",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/sla-escalations"),
      owner: cleanRiskOwner,
      breakingRisk: "高：升級路徑是營運承諾，欄位需向後相容",
      action: "把 SLA 升級固定成可交接資料；下一步補營運 KRI 落庫",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/operating-kri",
      ...
      auth: "internal",
      requestSchema: "{ portfolio_id?: string; batch_id?: string }",
      responseSchema: "OperatingKriItem[]",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/operating-kri"),
      owner: cleanRiskOwner,
      breakingRisk: "高：KRI 狀態會影響營運與風控決策，欄位需可追溯",
      action: "可查 BigQuery operating KRI board；下一步補市場警示落庫",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/operating-kri",
      ...
      version: "v1",
      auth: "internal",
      requestSchema: "OperatingKriSyncPayload & { workspace_id: string; portfolio_id: string; batch_id: string }",
      responseSchema: "OperatingKriSyncResult",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/operating-kri"),
      owner: cleanRiskOwner,
      breakingRisk: "高：KRI 是投委會與稽核依據，狀態與門檻需向後相容",
      action: "把營運 KRI 固定成可查詢資料；下一步補市場警示落庫",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/decision-funnel",
      ...
      auth: "internal",
      requestSchema: "{ portfolio_id?: string; batch_id?: string }",
      responseSchema: "DecisionFunnelStage[]",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/decision-funnel"),
      owner: cleanRiskOwner,
      breakingRisk: "高：階段狀態與轉換率會影響投委會決策與營運追蹤",
      action: "可查 BigQuery decision funnel board；下一步補 runbook/owner queue 落庫",
    },
    {
      method: "POST",
      endpoint: "/api/v1/trading/decision-funnel",
      ...
      version: "v1",
      auth: "internal",
      requestSchema: "DecisionFunnelSyncPayload & { workspace_id: string; portfolio_id: string; batch_id: string }",
      responseSchema: "DecisionFunnelSyncResult",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/decision-funnel"),
      owner: cleanRiskOwner,
      breakingRisk: "高：決策漏斗是資料到交易的主流程紀錄，欄位需向後相容",
      action: "把決策漏斗固定成可查詢資料；下一步補 runbook/owner queue 落庫",
    },
    {
      method: "GET",
      endpoint: "/api/v1/trading/market-alerts",
      ...
      auth: "internal",
      requestSchema: "{ portfolio_id?: string; batch_id?: string }",
      responseSchema: "MarketAlertEvent[]",
      stability: "draft",
    ...
    },
    {
      ...
      version: "v1",
      auth: "internal",
      requestSchema: "MarketAlertSyncPayload & { workspace_id: string; portfolio_id: string; batch_id: string }",
      responseSchema: "MarketAlertSyncResult",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/market-alerts"),
      owner: cleanRiskOwner,
    ...
    },
    {
      ...
      auth: "internal",
      requestSchema: "{ portfolio_id?: string; batch_id?: string }",
      responseSchema: "MarketAlertOwnerQueue[]",
      stability: "draft",
    ...
    },
    {
      ...
      version: "v1",
      auth: "internal",
      requestSchema: "MarketAlertOwnerQueueSyncPayload & { workspace_id: string; portfolio_id: string; batch_id: string }",
      responseSchema: "MarketAlertOwnerQueueSyncResult",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/market-alert-owner-queues"),
      owner: cleanRiskOwner,
    ...
    },
    {
      ...
      auth: "internal",
      requestSchema: "{ portfolio_id?: string; batch_id?: string }",
      responseSchema: "MarketAlertRunbookItem[]",
      stability: "draft",
    ...
    },
    {
      ...
      version: "v1",
      auth: "internal",
      requestSchema: "MarketAlertRunbookSyncPayload & { workspace_id: string; portfolio_id: string; batch_id: string }",
      responseSchema: "MarketAlertRunbookSyncResult",
      stability: "draft",
      status: serviceStatus("/api/v1/trading/market-alert-runbooks"),
      owner: cleanRiskOwner,
