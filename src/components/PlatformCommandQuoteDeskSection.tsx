import {
  ...
  type PlatformCommandQuoteDeskSummary,
...
type PlatformCommandQuoteDeskSectionProps = {
...
function statusBadgeClass(status: PlatformCommandQuoteDeskItem["status"]) {
...
export function PlatformCommandQuoteDeskSection({
  summary,
  items,
  onExportCsv: handleExportCsv,
}: PlatformCommandQuoteDeskSectionProps) {
  const metrics = [
    ["Quotes", `${summary.itemCount}`],
    ["Blocked", `${summary.blockedCount}`],
    ["Draft", `${summary.draftCount}`],
    ["Legal", `${summary.legalReviewCount}`],
    ["Ready", `${summary.readyCount}`],
    ["Risk Block", `${summary.blockedRiskCount}`],
    ["Review", `${summary.needsReviewCount}`],
    ["Clean", `${summary.readyRiskCount}`],
    ["Score", `${summary.averageReadinessScore}`],
    ["報價窗", summary.nextQuoteWindow],
  ];
    ...
    <div className="border-t border-slate-800 pt-3 space-y-3">
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h4 className="text-xs font-bold text-slate-100">Command Quote Desk 報價工作台</h4>
          ...
          <p className="mt-0.5 text-[11px] text-slate-500">
            把定價治理轉成 offer package、quote terms、法務檢查、entitlement plan 與 sales enablement
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-2">
        {items.slice(0, 3).map((item) => (
          <div key={`${item.quoteId}-card`} className="rounded-lg border border-slate-800 bg-slate-950 p-3">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="font-mono text-xs font-bold text-cyan-200">{item.quoteId}</p>
                <p className="mt-0.5 text-[10px] text-slate-600">{item.quoteOwner} / {item.quoteCadence}</p>
              </div>
            ...
            <p className="mt-3 text-[12px] font-bold text-slate-100">{item.offerPackage}</p>
            <div className="mt-3 grid grid-cols-3 gap-2 text-right text-xs">
              <div>
                <p className="text-[10px] text-slate-600">Desk</p>
                <p className="font-bold text-slate-100">{platformCommandQuoteDeskStateLabel(item.quoteDeskState)}</p>
              </div>
              <div>
            ...
            <p className="mt-3 text-[11px] text-slate-300">{item.quoteTerms}</p>
            <p className="mt-1 text-[11px] text-amber-200">{item.salesEnablement}</p>
          </div>
        ))}
      </div>

      <div className="overflow-x-auto rounded-lg border border-slate-800">
        <table className="w-full min-w-[2460px] text-xs">
          <thead className="bg-slate-900/80">
            <tr className="text-left text-[11px] text-slate-600">
              ...
              <th className="py-2 px-3 font-medium">Quote Owner</th>
              <th className="py-2 px-3 font-medium">Quote Desk</th>
              <th className="py-2 px-3 font-medium">Risk Signal</th>
              ...
              <th className="py-2 px-3 font-medium">Offer Package</th>
              <th className="py-2 px-3 font-medium">Quote Terms</th>
              <th className="py-2 px-3 font-medium">Legal Note</th>
              <th className="py-2 px-3 font-medium">Approval Packet</th>
              <th className="py-2 px-3 font-medium">Entitlement Plan</th>
              <th className="py-2 px-3 font-medium">Delivery Handoff</th>
              <th className="py-2 px-3 font-medium">Sales Enablement</th>
              <th className="py-2 px-3 font-medium">Buyer Risk</th>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={`${item.quoteId}-row`} className="border-t border-slate-800/80">
                <td className="py-2 px-3 font-mono font-bold text-cyan-200">{item.quoteId}</td>
                ...
                <td className="py-2 px-3 text-cyan-200">{item.quoteOwner}</td>
                ...
                <td className="py-2 px-3 text-slate-300">{item.offerPackage}</td>
                <td className="py-2 px-3 text-amber-200">{item.quoteTerms}</td>
                <td className="py-2 px-3 text-slate-400">{item.legalNote}</td>
                <td className="py-2 px-3 text-slate-400">{item.approvalPacket}</td>
                <td className="py-2 px-3 text-cyan-200">{item.entitlementPlan}</td>
                <td className="py-2 px-3 text-slate-300">{item.deliveryHandoff}</td>
                <td className="py-2 px-3 text-amber-200">{item.salesEnablement}</td>
                <td className="py-2 px-3 text-slate-500">{item.buyerRisk}</td>
                <td className="py-2 px-3 text-slate-300">{item.quoteCadence}</td>
