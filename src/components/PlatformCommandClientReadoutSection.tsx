import {
  ...
  type PlatformCommandClientReadoutSummary,
...
type PlatformCommandClientReadoutSectionProps = {
...
function statusBadgeClass(status: PlatformCommandClientReadoutItem["status"]) {
...
export function PlatformCommandClientReadoutSection({
  summary,
  items,
  onExportCsv: handleExportCsv,
}: PlatformCommandClientReadoutSectionProps) {
  const metrics = [
    ["Readouts", `${summary.itemCount}`],
    ["Blocked", `${summary.blockedCount}`],
    ["Drafting", `${summary.draftingCount}`],
    ["Review", `${summary.reviewCount}`],
    ["Deliverable", `${summary.deliverableCount}`],
    ["Restricted", `${summary.restrictedCount}`],
    ["Internal", `${summary.internalOnlyCount}`],
    ["Client ready", `${summary.clientReadyCount}`],
    ...
    <div className="border-t border-slate-800 pt-3 space-y-3">
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h4 className="text-xs font-bold text-slate-100">Command Client Readout Pack 客戶交付簡報包</h4>
          ...
          <p className="mt-0.5 text-[11px] text-slate-500">
            把董事會報告包轉成客戶摘要、揭露等級、交付通道、免責文字、附件位置與下一步
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-2">
        {items.slice(0, 3).map((item) => (
          <div key={`${item.readoutId}-card`} className="rounded-lg border border-slate-800 bg-slate-950 p-3">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="font-mono text-xs font-bold text-cyan-200">{item.readoutId}</p>
                <p className="mt-0.5 text-[10px] text-slate-600">{item.clientSegment} / {item.deliveryWindow}</p>
              </div>
            ...
            <p className="mt-3 text-[12px] font-bold text-slate-100">{item.clientHeadline}</p>
            ...
            <p className="mt-3 text-[11px] text-slate-300">{item.permittedMessage}</p>
            <p className="mt-1 text-[11px] text-amber-200">{item.requiredDisclaimer}</p>
          </div>
        ))}
      </div>
        ...
        <table className="w-full min-w-[2380px] text-xs">
          <thead className="bg-slate-900/80">
            <tr className="text-left text-[11px] text-slate-600">
              ...
              <th className="py-2 px-3 font-medium">Client Owner</th>
              ...
              <th className="py-2 px-3 font-medium">Disclaimer</th>
              <th className="py-2 px-3 font-medium">Client Action</th>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={`${item.readoutId}-row`} className="border-t border-slate-800/80">
                <td className="py-2 px-3 font-mono font-bold text-cyan-200">{item.readoutId}</td>
                <td className="py-2 px-3 font-mono text-slate-300">{item.reportId}</td>
                ...
                <td className="py-2 px-3 text-cyan-200">{item.clientOwner}</td>
                ...
                <td className="py-2 px-3 text-slate-400">{item.clientSegment}</td>
                <td className="py-2 px-3 text-slate-300">{item.clientHeadline}</td>
                <td className="py-2 px-3 text-slate-300">{item.permittedMessage}</td>
                <td className="py-2 px-3 text-slate-500">{item.disclosureLevel}</td>
                <td className="py-2 px-3 text-slate-400">{item.deliveryChannel}</td>
                <td className="py-2 px-3 text-amber-200">{item.requiredDisclaimer}</td>
                <td className="py-2 px-3 text-amber-200">{item.clientAction}</td>
                <td className="py-2 px-3 text-slate-300">{item.deliveryWindow}</td>
                <td className="py-2 px-3 font-mono text-cyan-200">{item.appendixLocation}</td>
