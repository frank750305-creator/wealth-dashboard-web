import {
  ...
  type PlatformCommandRevenueOperationsLedgerSummary,
...
type PlatformCommandRevenueOperationsLedgerSectionProps = {
...
function statusBadgeClass(status: PlatformCommandRevenueOperationsLedgerItem["status"]) {
...
export function PlatformCommandRevenueOperationsLedgerSection({
  summary,
  items,
  onExportCsv: handleExportCsv,
}: PlatformCommandRevenueOperationsLedgerSectionProps) {
  const metrics = [
    ["RevOps", `${summary.itemCount}`],
    ["Blocked", `${summary.blockedCount}`],
    ["Contract", `${summary.contractReviewCount}`],
    ["Billing", `${summary.billingSetupCount}`],
    ["Live", `${summary.revenueLiveCount}`],
    ["Defer", `${summary.deferCount}`],
    ["Manual", `${summary.manualReviewCount}`],
    ["Recognize", `${summary.recognizableCount}`],
    ["Score", `${summary.averageReadinessScore}`],
    ["月結窗", summary.nextCloseWindow],
  ];
    ...
    <div className="border-t border-slate-800 pt-3 space-y-3">
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h4 className="text-xs font-bold text-slate-100">Command Revenue Operations Ledger 收入營運台帳</h4>
          ...
          <p className="mt-0.5 text-[11px] text-slate-500">
            把續約 forecast 轉成合約、帳單、ARR / MRR、收入確認、收款風險與 RevOps 月結控制
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-2">
        {items.slice(0, 3).map((item) => (
          <div key={`${item.revOpsId}-card`} className="rounded-lg border border-slate-800 bg-slate-950 p-3">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="font-mono text-xs font-bold text-cyan-200">{item.revOpsId}</p>
                <p className="mt-0.5 text-[10px] text-slate-600">{item.revOpsOwner} / {item.closeWindow}</p>
              </div>
            ...
            <p className="mt-3 text-[12px] font-bold text-slate-100">{item.contractStatus}</p>
            <div className="mt-3 grid grid-cols-3 gap-2 text-right text-xs">
              <div>
                <p className="text-[10px] text-slate-600">State</p>
                <p className="font-bold text-slate-100">{platformCommandRevOpsStateLabel(item.revOpsState)}</p>
              </div>
              <div>
            ...
            <p className="mt-3 text-[11px] text-slate-300">{item.arrTreatment}</p>
      ...
      <div className="overflow-x-auto rounded-lg border border-slate-800">
        <table className="w-full min-w-[2420px] text-xs">
          <thead className="bg-slate-900/80">
            <tr className="text-left text-[11px] text-slate-600">
              ...
              <th className="py-2 px-3 font-medium">RevOps Owner</th>
              ...
              <th className="py-2 px-3 font-medium">ARR</th>
              <th className="py-2 px-3 font-medium">MRR</th>
              <th className="py-2 px-3 font-medium">Recognition Note</th>
              <th className="py-2 px-3 font-medium">Collection Risk</th>
              <th className="py-2 px-3 font-medium">Finance Control</th>
              <th className="py-2 px-3 font-medium">Cadence</th>
              <th className="py-2 px-3 font-medium">Close Window</th>
              <th className="py-2 px-3 font-medium">Next Action</th>
            </tr>
          </thead>
          <tbody>
            {items.map((item) => (
              <tr key={`${item.revOpsId}-row`} className="border-t border-slate-800/80">
                <td className="py-2 px-3 font-mono font-bold text-cyan-200">{item.revOpsId}</td>
                ...
                <td className="py-2 px-3 text-cyan-200">{item.revOpsOwner}</td>
                ...
                <td className="py-2 px-3 text-slate-300">{item.contractStatus}</td>
                <td className="py-2 px-3 text-slate-300">{item.billingStatus}</td>
                <td className="py-2 px-3 text-cyan-200">{item.arrTreatment}</td>
                <td className="py-2 px-3 text-slate-400">{item.mrrMovement}</td>
                <td className="py-2 px-3 text-slate-500">{item.revenueRecognitionNote}</td>
                <td className="py-2 px-3 text-amber-200">{item.collectionRisk}</td>
                <td className="py-2 px-3 text-slate-400">{item.financeControl}</td>
                <td className="py-2 px-3 text-slate-300">{item.operatingCadence}</td>
