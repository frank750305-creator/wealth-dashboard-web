import {
  ...
  type PlatformCommandAuditTrailSummary,
...
type PlatformCommandAuditTrailSectionProps = {
  summary: PlatformCommandAuditTrailSummary;
  items: PlatformCommandAuditTrailItem[];
  onExportCsv: () => void;
};

function auditStateBadgeClass(state: PlatformCommandAuditState) {
...
function statusBadgeClass(status: PlatformCommandAuditTrailItem["status"]) {
...
export function PlatformCommandAuditTrailSection({
  summary,
  items,
  onExportCsv: handleExportCsv,
}: PlatformCommandAuditTrailSectionProps) {
  const metrics = [
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h4 className="text-xs font-bold text-slate-100">Command Audit Trail 稽核軌跡</h4>
          ...
          <p className="mt-0.5 text-[11px] text-slate-500">
            把證據台帳轉成稽核狀態、控制目標、抽樣證據、補救動作與簽核保存位置
          </p>
        </div>
          ...
          <div key={`${item.auditId}-card`} className="rounded-lg border border-slate-800 bg-slate-950 p-3">
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="mt-0.5 text-[10px] text-slate-600">{item.auditOwner} / {item.auditWindow}</p>
              </div>
              <span className={`rounded px-2 py-0.5 text-[10px] font-bold ${auditStateBadgeClass(item.auditState)}`}>
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 text-right text-xs">
              <div>
                ...
                <p className="font-bold text-slate-100">{platformCommandAuditResultLabel(item.auditResult)}</p>
              </div>
              <div>
            ...
            <p className="mt-3 text-[11px] text-slate-300">{item.controlObjective}</p>
            <p className="mt-1 text-[11px] text-amber-200">{item.remediationAction}</p>
          </div>
        ))}
      </div>

      <div className="overflow-x-auto rounded-lg border border-slate-800">
        <table className="w-full min-w-[2140px] text-xs">
          <thead className="bg-slate-900/80">
            <tr className="text-left text-[11px] text-slate-600">
              ...
              <th className="py-2 px-3 font-medium">Control Objective</th>
              <th className="py-2 px-3 font-medium">Sampled Evidence</th>
              ...
              <th className="py-2 px-3 font-medium">Next Event</th>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={`${item.auditId}-row`} className="border-t border-slate-800/80">
                ...
                <td className="py-2 px-3 text-slate-300">{item.controlObjective}</td>
                <td className="py-2 px-3 text-slate-500">{item.sampledEvidence}</td>
                <td className="py-2 px-3 text-amber-200">{item.auditFinding}</td>
                <td className="py-2 px-3 text-amber-200">{item.remediationAction}</td>
                <td className="py-2 px-3 text-slate-300">{item.auditWindow}</td>
                <td className="py-2 px-3 text-cyan-200">{item.nextAuditEvent}</td>
                <td className="py-2 px-3 text-slate-400">{item.signoffOwner}</td>
                <td className="py-2 px-3 font-mono text-cyan-200">{item.archiveLocation}</td>
              </tr>
            ))}
          </tbody>
        </table>
