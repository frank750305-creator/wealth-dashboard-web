import {
  ...
  type PlatformCommandDecisionFollowUpSummary,
...
type PlatformCommandDecisionFollowUpSectionProps = {
...
function statusBadgeClass(status: PlatformCommandDecisionFollowUpItem["status"]) {
...
export function PlatformCommandDecisionFollowUpSection({
  summary,
  items,
  onExportCsv: handleExportCsv,
}: PlatformCommandDecisionFollowUpSectionProps) {
  const metrics = [
    ["Follow-up", `${summary.itemCount}`],
    ...
    ["追蹤窗", summary.nextFollowUpWindow],
  ];
    ...
    <div className="border-t border-slate-800 pt-3 space-y-3">
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h4 className="text-xs font-bold text-slate-100">Command Decision Follow-up 決策落地追蹤</h4>
          ...
          <p className="mt-0.5 text-[11px] text-slate-500">
            把決策登錄轉成落地狀態、證據要求、升級路徑、下一次追蹤與 closure gate
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-2">
        {items.slice(0, 3).map((item) => (
          <div key={`${item.decisionId}-followup-card`} className="rounded-lg border border-slate-800 bg-slate-950 p-3">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="text-xs font-bold text-slate-100">{item.decisionId}</p>
                <p className="mt-0.5 text-[10px] text-slate-600">{item.followUpOwner} / {item.dueWindow}</p>
              </div>
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 text-right text-xs">
              ...
              <div>
                <p className="text-[10px] text-slate-600">Decision</p>
                <p className="font-bold text-slate-100">{platformCommandDecisionStateLabel(item.decisionState)}</p>
              </div>
              <div>
            ...
            <p className="mt-3 text-[11px] text-amber-200">{item.actionPlan}</p>
            <p className="mt-1 text-[11px] text-slate-500">{item.closureGate}</p>
          </div>
        ))}
      </div>

      <div className="overflow-x-auto rounded-lg border border-slate-800">
        <table className="w-full min-w-[2120px] text-xs">
          <thead className="bg-slate-900/80">
            <tr className="text-left text-[11px] text-slate-600">
              <th className="py-2 px-3 font-medium">Decision ID</th>
              <th className="py-2 px-3 font-medium">Owner</th>
              <th className="py-2 px-3 font-medium">Follow-up Owner</th>
              ...
              <th className="py-2 px-3 font-medium">Action Plan</th>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={`${item.decisionId}-followup-row`} className="border-t border-slate-800/80">
                ...
                <td className="py-2 px-3 text-slate-400">{item.followUpOwner}</td>
                ...
                <td className="py-2 px-3 text-amber-200">{item.actionPlan}</td>
                <td className="py-2 px-3 text-slate-300">{item.dueWindow}</td>
                <td className="py-2 px-3 text-slate-500">{item.proofRequired}</td>
                <td className="py-2 px-3 text-slate-500">{item.successCriteria}</td>
                <td className="py-2 px-3 text-cyan-200">{item.escalationPath}</td>
                <td className="py-2 px-3 text-slate-300">{item.nextReview}</td>
